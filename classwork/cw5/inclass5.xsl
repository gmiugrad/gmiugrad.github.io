...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
    ...
    <xsl:template match="name">
     <xsl:element name="h1"><xsl:text>Hello, my name is </xsl:text><xsl:apply-templates/></xsl:element>   
    </xsl:template>
    ...
    <xsl:template match="description">
        ...
        <xsl:element name="p"><xsl:text>I am a </xsl:text><xsl:apply-templates/></xsl:element>
        
    </xsl:template>
    
    <xsl:template match="personality">
        
        <xsl:element name="p"><xsl:text>My personality is </xsl:text><xsl:apply-templates/></xsl:element>
        
    </xsl:template>
    
    <xsl:template match="power-level">
        <xsl:element name="p"><xsl:text>My power level is </xsl:text><xsl:apply-templates/></xsl:element>
        
    </xsl:template>
    
    <xsl:template match="memento">
        <xsl:element name="p"><xsl:text>My memento is a </xsl:text><xsl:apply-templates/></xsl:element>
    </xsl:template>
